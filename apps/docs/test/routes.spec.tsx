    ...
    {},
    {
      get: (_target, tagName: string) =>
        React.forwardRef(function MotionComponent(
          ...
          const { animate, initial, layout, transition, variants, viewport, whileHover, whileInView, ...safeProps } =
            props;
...
describe('docs routes', () => {
  it('renders the home page with package links', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: /shared building blocks behind this javascript library monorepo/i })
    ).toBeInTheDocument();
  ...
  it('renders a library package page with quick-start and reference sections', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: /Pull utility functions and hooks from the package root/i, level: 2 })
    ).toBeInTheDocument();
